...
// ─── Unicode text wrapper (R4 fix) ─────────────────────────────────────
// jsPDF's doc.text() uses whatever font is currently set via setFont(). The
// 12 template renderers set 'helvetica' and never switch, so any non-CP1252
// text (CJK, Arabic, Hebrew, etc.) renders as blank boxes. Rather than edit
// all ~80 doc.text call sites across 12 renderers, we wrap doc.text on the
// instance: for each call, inspect the text argument(s), pick the font that
// can render them (Helvetica for pure CP1252, NotoSans otherwise), set it
// preserving the caller's intended style ('bold'/'italic' where Helvetica
// supports it — NotoSans only ships Regular so non-Latin bold falls back to
// regular weight), delegate to the original text(), then restore the prior
// font so the caller's subsequent setFont expectations are unaffected.
//
// autoTable is handled separately: it does NOT call doc.text for its cells
// (it builds text runs internally), so this wrapper doesn't intercept it.
// Each autoTable call passes a `didDrawCell`/`styles` hook OR we rely on
// autoTable's per-cell font via styles — see installAutoTableFontFallback.
type TextArg = string | string[];
const ORIGINAL_TEXT = Symbol('originalText');

function installTextFontFallback(doc: jsPDF): void {
  const anyDoc = doc as unknown as {
    [ORIGINAL_TEXT]: typeof doc.text;
    text: typeof doc.text;
  };
  if (anyDoc[ORIGINAL_TEXT]) return; // already wrapped
  const original = doc.text.bind(doc);
  anyDoc[ORIGINAL_TEXT] = original;

  anyDoc.text = function (text: TextArg, x: number, y: number, options?: any) {
    const prior = doc.getFont();
    const priorName = prior.fontName;
    const priorStyle = prior.fontStyle;
    // Determine target font/style. If text is an array, use the union of all
    // lines — if ANY line has a non-CP1252 char, use NotoSans for the whole
    // call (jsPDF can't mix fonts within one text() invocation).
    const strings: string[] = Array.isArray(text) ? text : [text];
    const wantNoto = strings.some((s) => selectFontFor(doc, String(s)) !== 'helvetica');
    const family = wantNoto ? 'NotoSans' : 'helvetica';
    // NotoSans only has 'normal'; map bold/italic → normal for it. Helvetica
    // supports normal/bold/italic/bolditalic.
    const style = wantNoto ? 'normal' : priorStyle;
    doc.setFont(family, style);
    try {
      return original(text as any, x, y, options);
    } finally {
      // Restore so caller code that assumes its own setFont() is still active
      // continues to work (most renderers set the font right before text()).
      doc.setFont(priorName, priorStyle);
    }
  } as typeof doc.text;
}

// Restore the original doc.text (used by tests that want raw behavior).
export function uninstallTextFontFallback(doc: jsPDF): void {
  const anyDoc = doc as unknown as {
    [ORIGINAL_TEXT]?: typeof doc.text;
    text: typeof doc.text;
  };
  if (anyDoc[ORIGINAL_TEXT]) {
    anyDoc.text = anyDoc[ORIGINAL_TEXT];
    delete anyDoc[ORIGINAL_TEXT];
  }
}
...
export function generatePDF(invoice: Invoice): Blob {
  ...
  // R4 fix: Unicode font support. jsPDF's built-in Helvetica only renders
  // CP1252 (Latin-1 + a few extensions); every other script (CJK, Arabic,
  // Hebrew, Cyrillic beyond Latin-1, Greek, emoji) shows as blank boxes.
  // We register a merged Noto Sans Regular TTF (Latin/Cyrillic/Greek/Arabic/
  // Hebrew/CJK/Hiragana, ~32k glyphs) and wrap doc.text so each call picks
  // Helvetica when the string is fully CP1252 (smaller, crisper — the common
  // case for English/Western-European invoices) and falls back to Noto Sans
  // when any character is outside that range. The 12 template renderers call
  // doc.text directly, so wrapping it here fixes all of them in one place
  // without touching each renderer. autoTable receives its own per-cell font
  // selection via its styles hook (see the autoTable calls in renderers).
  registerUnicodeFont(doc);
  installTextFontFallback(doc);
